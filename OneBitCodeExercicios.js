...
for(let i = 0; i < 2; i++){
    veiculos [i] = window.prompt("Isira o nome do veiculo:");
    velocidade[i] = window.prompt("Informe a velocidade");
}

for(let i = 0; i < 2; i++){
    console.log(`Modelo: ${veiculos[i]} \nVelocidade: ${velocidade[i]} Km/h`)
}

console.log (
      velocidade[0] > velocidade[1] ? `${veiculos[0]} é mais rapido que ${veiculos[1]}` :
      velocidade[0] < velocidade [1] ? `${veiculos[1]} é mais rapido que ${veiculos[0]}` : "velocidades são iguais"
    );
